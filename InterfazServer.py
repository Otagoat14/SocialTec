...
class ServerGUI:
    ...
    def buscar_path(self):
        ...
        if origen not in self.grafo.grafo:
            messagebox.showwarning("Error", f"El usuario '{origen}' no existe en el grafo")
            return
        
        if destino not in self.grafo.grafo:
            messagebox.showwarning("Error", f"El usuario '{destino}' no existe en el grafo")
            return

        existe, path = self.grafo.existe_path(origen, destino)

        if existe:
            messagebox.showinfo("Info", f" Sí existe path:\n{','.join(path)}")
        else:
